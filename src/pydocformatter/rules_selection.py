...
from pydocformatter.rules.base import RULE_SELECTOR_RE, RuleMetadata
from pydocformatter.rules.collection import ALL_RULE_CODE, RuleCollection, collect_rules
...
def select_rules(settings: CheckSettings, *, collection: RuleCollection | None = None) -> RuleSelection:
    ...
    selected_rules = tuple(SelectedRule(rule=rule, fixable=rule.fixable and rule.code in effectively_fixable_codes) for rule in collection.rules if rule.code in enabled_codes)
    ...
    *,
    ...
    for selector in selectors:
        ...
        if require_inherently_fixable:
            fixable_rules = tuple(rule for rule in matching_rules if rule.fixable)
            if not fixable_rules and selector != ALL_RULE_CODE:
                errors.append(f"{context} selector {selector!r} only matches inherently unfixable rules")
            matching_rules = fixable_rules

        rule_codes.update(rule.code for rule in matching_rules)
    return rule_codes


def _matching_rules(selector: str, *, collection: RuleCollection, context: str, errors: list[str]) -> tuple[RuleMetadata, ...]:
    """Resolve one selector against a collection."""
    if selector != ALL_RULE_CODE and RULE_SELECTOR_RE.fullmatch(selector) is None:
        errors.append(f"{context} contains invalid selector: {selector}")
        return ()

    matching_rules = collection.matching_rules(selector)
    if not matching_rules and selector != ALL_RULE_CODE:
