...
def _planned_change_for_docstring(docstring: PDF_definition.DocstringInfo) -> rule_edits.PlannedSourceChange | None:
    ...
    if content_end == len(line.raw_text):
        return None
    return (
        _validated_change(docstring, line, content_end=content_end, replacement_text="")
        or _escaped_quote_change(docstring, line, content_end=content_end)
        or _validated_change(docstring, line, content_end=content_end, replacement_text=" ")
    )


def _escaped_quote_change(docstring: PDF_definition.DocstringInfo, line: PDF_definition.DocstringValueLine, *, content_end: int) -> rule_edits.PlannedSourceChange | None:
...
def _validated_change(docstring: PDF_definition.DocstringInfo, line: PDF_definition.DocstringValueLine, *, content_end: int, replacement_text: str) -> rule_edits.PlannedSourceChange | None:
