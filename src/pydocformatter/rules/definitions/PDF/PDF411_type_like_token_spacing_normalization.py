...
def _line_replacement(
    convention: DocstringConvention, line: PDF_definition.DocstringValueLine, entry: PDF_definition.DocstringEntry, *, normalized_type_cache: dict[str, str | None]
...
def _google_replacement(line: PDF_definition.DocstringValueLine, entry: PDF_definition.DocstringEntry, *, normalized_type_cache: dict[str, str | None]) -> _LineReplacement | None:
    ...
    if entry.kind in {PDF_definition.DocstringEntryKind.RETURN, PDF_definition.DocstringEntryKind.YIELD}:
        generic_match = PDF_definition._match_generic_entry(line.text)
        if generic_match is not None and entry.type_text is not None:
            return _normalized_replacement(line, generic_match.name_start, generic_match.name_end, entry.type_text, normalized_type_cache=normalized_type_cache)
    return None
...
def _normalized_replacement(line: PDF_definition.DocstringValueLine, start_column: int, end_column: int, text: str, *, normalized_type_cache: dict[str, str | None]) -> _LineReplacement | None:
