...
| `PDF4xx` | Convention structure                      | Section names, headers, underlines, content, order, entry syntax, entry indentation, NumPy return shape, spacing, punctuation, and conservative type spelling.                                          |
| `PDF5xx` | Docstring/signature validation            | Parameter presence, extraneous names, configured variadic spelling, and relative declaration order plus return, yield, exception, and attribute documentation consistency.                              |
