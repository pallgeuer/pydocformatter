...
PCF rules never format shebangs or valid first- or second-line encoding cookies. Type comments and recognized tool directives are protected from ordinary comment formatting; PCF003 may still normalize only the harmless spacing around known trailing directives. Protected tool directives include `noqa`, `nosec`, `nosemgrep`, `pylint`, `pyright`, `mypy`, `ruff`, `flake8`, `fmt:`, `isort:`, and `pragma` comments.
...
- `PCF003` normalizes safe spacing around known trailing directive comments without changing directive payloads.
...
| `comment-syntax-aware-trailing-extraction` |  `true` | Keep overlong trailing comments inline in decorators, compound headers, arguments, and continuation contexts. |
