...
Rule is incompatible with `PCF102`.
...
Known exact pydocfmt names can be replaced automatically while preserving the surrounding directive syntax and unrelated selectors. Code and name aliases for a converted local rule are deduplicated by their shared identity, while unrelated duplicates remain unchanged; PCF100 owns general list deduplication when selected. Broad pydocfmt selectors are already code selectors and remain permitted; unknown and invalid pydocfmt selectors are left to PCF101. Comments with unrecognized pydocfmt actions are ignored.
...
This rule is the inverse local policy to PCF102. Ruff does not provide an equivalent rule that requires names to be converted to codes in suppression comments.
...
[output]
# pydocfmt: file-ignore[PDF, PDF101, PCF000, future-rule, bad!]  # generated
value = 1
```
...
[output=unchanged]
[findings]
PCF103: Line 1: Suppression comment should use rule codes instead of names
PCF103: Line 3: Suppression comment should use rule codes instead of names
PCF103: Line 5: Suppression comment should use rule codes instead of names
```
