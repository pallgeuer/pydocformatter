...
## Ruff compatibility
Ruff can report overlong comment lines and general whitespace issues, but does not provide equivalent configurable standalone-comment reflow. PCF000 complements those checks and should not be used to rewrite Ruff, type-checker, formatter, or security directives, which the PCF category protects.
...
Ambiguous supported-markup prefixes are not reflowed. PCF000 can still fix marker spacing without changing the semantic body when that is the only canonical difference, while an independently overlong canonical line retains an unfixable finding until the ambiguity is resolved:
...
[findings]
PCF000: Line 2: Standalone comment needs formatting
```
