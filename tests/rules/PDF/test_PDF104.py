...
def test_preserves_internal_blank_lines_raw_prefix_quote_delimiter_and_crlf() -> None:
    ...
    settings = CheckSettings(select=("PDF104",), line_ending=LineEnding.CR_LF)
...
def test_escapes_quote_collision_before_same_line_closing_quotes() -> None:
    ...
    assert result.fixed_findings[PDF104MultilineClosingQuotesSameLine.meta] == 3
    assert not result.unfixed_findings
    assert not format_pdf104(result.new_source).modified
...
def test_check_fix_line_numbers_and_fix_false_findings_agree() -> None:
    ...
    findings = PDF104MultilineClosingQuotesSameLine.check(context)
    fixed = PDF104MultilineClosingQuotesSameLine.fix(context)
    ...
    assert PDF104MultilineClosingQuotesSameLine.check(fixed_context) == ()
