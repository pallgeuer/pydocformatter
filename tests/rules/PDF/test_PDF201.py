...
def test_nested_final_numpy_section_has_one_configured_blank(existing_blank: bool) -> None:
    blank = "\n" if existing_blank else ""
    source = f'"""\nNotes\n Notes\n{blank}"""\n'
    settings = CheckSettings(select=("PDF201",), docstring_convention=DocstringConvention.NUMPY, docstring_blank_line_after_last_section=True)
    result = format_source(source, settings=settings)

    assert result.new_source == '"""\nNotes\n Notes\n\n"""\n'
    assert bool(result.fixed_findings) is not existing_blank
