...
from pydocformatter.rules.definitions.PDF.PDF006_closing_quotes_whitespace import PDF006ClosingQuotesWhitespace
...
def contexts(source: str, *, settings: CheckSettings | None = None) -> tuple[RuleCategoryContext, RuleContext]:
    ...
    category = RuleCategoryContext(
        path="example.py",
        settings=CheckSettings(select=("PDF006",)) if settings is None else settings,
        module=module,
...
def format_pdf006(source: str, *, settings: CheckSettings | None = None, fix: bool = True) -> formatter.FormatterResult:
    """Format source with PDF006 selected."""
    resolved_settings = CheckSettings(select=("PDF006",)) if settings is None else settings
...
def test_module_parenthesized_simple_suite_and_neutral_settings() -> None:
    ...
    settings = CheckSettings(select=("PDF006",), docstring_convention=DocstringConvention.NUMPY, indent_style=IndentStyle.TAB, indent_width=8)
...
def test_check_fix_line_numbers_and_fix_false_findings_agree() -> None:
    ...
    findings = PDF006ClosingQuotesWhitespace.check(context)
    fixed = PDF006ClosingQuotesWhitespace.fix(context)
    ...
    assert PDF006ClosingQuotesWhitespace.check(fixed_context) == ()
...
def test_preserves_raw_prefix_quote_delimiter_non_ascii_and_crlf_line_endings() -> None:
    ...
    result = format_pdf006(source, settings=CheckSettings(select=("PDF006",), line_ending=LineEnding.CR_LF))

    assert result.new_source == "def function():\r\n    r'''Path C:\\\\temp and caf\xe9.'''\r\n"
    assert not format_pdf006(result.new_source, settings=CheckSettings(select=("PDF006",), line_ending=LineEnding.CR_LF)).modified


def test_pdf000_can_literalize_escaped_newline_before_pdf006_trims_it() -> None:
    source = 'def function():\n    """Summary.\\nBody.  """\n'
    settings = CheckSettings(select=("PDF000", "PDF006"))
...
def test_pdf005_and_pdf006_can_normalize_both_sides() -> None:
    source = 'def function():\n    """  Summary.  """\n'
    settings = CheckSettings(select=("PDF005", "PDF006"))
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
