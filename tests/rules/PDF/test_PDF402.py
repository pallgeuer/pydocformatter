...
from pydocformatter.rules.definitions.PDF.PDF402_section_name_trailing_colon import PDF402SectionNameTrailingColon
...
def test_adds_missing_google_section_name_colon() -> None:
    ...
    assert result.fixed_findings[PDF402SectionNameTrailingColon.meta] == 1
    assert PDF402SectionNameTrailingColon.meta.name == "section-name-trailing-colon"
...
def test_ignored_outside_google_convention() -> None:
    source = 'def function(value):\n    """Summary.\n\n    Args\n        value: Description.\n    """\n'
    result = format_source(source, settings=CheckSettings(select=("PDF402",), docstring_convention=DocstringConvention.NUMPY))
