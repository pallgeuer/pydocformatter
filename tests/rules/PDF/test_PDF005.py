...
from pydocformatter.rules.definitions.PDF.PDF005_opening_quotes_whitespace import PDF005OpeningQuotesWhitespace
...
def contexts(source: str, *, settings: CheckSettings | None = None) -> tuple[RuleCategoryContext, RuleContext]:
    ...
    category = RuleCategoryContext(
        path="example.py",
        settings=CheckSettings(select=("PDF005",)) if settings is None else settings,
        module=module,
...
def format_pdf005(source: str, *, settings: CheckSettings | None = None, fix: bool = True) -> formatter.FormatterResult:
    """Format source with PDF005 selected."""
    resolved_settings = CheckSettings(select=("PDF005",)) if settings is None else settings
...
def test_removes_opening_whitespace_from_one_line_and_multiline_docstrings() -> None:
    ...
    assert result.fixed_findings[PDF005OpeningQuotesWhitespace.meta] == 2
...
def test_non_space_tab_whitespace_counts_as_content() -> None:
    ...
    assert result.fixed_findings[PDF005OpeningQuotesWhitespace.meta] == 1
...
def test_trims_before_leading_delimiter_quotes() -> None:
    ...
    assert result.fixed_findings[PDF005OpeningQuotesWhitespace.meta] == 4
...
def test_module_parenthesized_simple_suite_and_neutral_settings() -> None:
    ...
    settings = CheckSettings(select=("PDF005",), docstring_convention=DocstringConvention.GOOGLE, indent_style=IndentStyle.TAB, indent_width=8)
    result = format_pdf005(source, settings=settings)

    assert result.new_source == '"""Module."""\n\nclass Parenthesized:\n    ("""Class.""")\n\ndef simple(): """Summary."""; return None\n'
    assert result.fixed_findings[PDF005OpeningQuotesWhitespace.meta] == 3
...
def test_check_fix_line_numbers_and_fix_false_findings_agree() -> None:
    ...
    findings = PDF005OpeningQuotesWhitespace.check(context)
    fixed = PDF005OpeningQuotesWhitespace.fix(context)
    ...
    assert PDF005OpeningQuotesWhitespace.check(fixed_context) == ()
...
def test_preserves_raw_prefix_quote_delimiter_non_ascii_and_crlf_line_endings() -> None:
    ...
    result = format_pdf005(source, settings=CheckSettings(select=("PDF005",), line_ending=LineEnding.CR_LF))

    assert result.new_source == "def function():\r\n    r'''Path C:\\\\temp and caf\xe9.'''\r\n"
    assert not format_pdf005(result.new_source, settings=CheckSettings(select=("PDF005",), line_ending=LineEnding.CR_LF)).modified


def test_pdf000_can_literalize_escaped_newline_before_pdf005_trims_it() -> None:
    source = 'def function():\n    """  Summary.\\nBody."""\n'
    settings = CheckSettings(select=("PDF000", "PDF005"))
