...
from pydocformatter.rules.definitions.PDF.PDF004_docstring_blank_line_whitespace import PDF004DocstringBlankLineWhitespace
...
def contexts(source: str, *, settings: settings_check.CheckSettings | None = None) -> tuple[RuleCategoryContext, RuleContext]:
    ...
    category = RuleCategoryContext(
        path="example.py",
        settings=settings_check.CheckSettings(select=("PDF004",)) if settings is None else settings,
        module=module,
...
def format_pdf004(source: str, *, settings: settings_check.CheckSettings | None = None, fix: bool = True) -> formatter.FormatterResult:
    """Format source with PDF004 selected."""
    resolved_settings = settings_check.CheckSettings(select=("PDF004",)) if settings is None else settings
...
def test_default_blank_style_makes_ordinary_blank_lines_truly_blank() -> None:
    ...
    assert result.fixed_findings[PDF004DocstringBlankLineWhitespace.meta] == 1
...
def test_multiple_blank_line_kinds_in_one_docstring_are_reported_together() -> None:
    ...
    findings = PDF004DocstringBlankLineWhitespace.check(context)
    result = format_pdf004(source)
...
def test_aligned_style_aligns_ordinary_blank_lines_to_canonical_margin() -> None:
    source = 'def function():\n    """Summary.\n\n    Body.\n    """\n'
    settings = settings_check.CheckSettings(select=("PDF004",), docstring_blank_line_style=settings_check.DocstringBlankLineStyle.ALIGNED)
...
def test_aligned_style_simple_suite_uses_configured_tab_indent_unit() -> None:
    ...
    settings = settings_check.CheckSettings(select=("PDF004",), docstring_blank_line_style=settings_check.DocstringBlankLineStyle.ALIGNED, indent_style=settings_check.IndentStyle.TAB)
    result = format_pdf004(source, settings=settings)
...
def test_whitespace_only_single_line_docstrings_are_final_lines() -> None:
    source = '"""   """\n\ndef block_suite():\n    """   """\n\ndef simple_suite(): """   """\n'
    result = format_pdf004(source, settings=settings_check.CheckSettings(select=("PDF004",), indent_width=2))
...
def test_empty_docstrings_are_not_treated_as_closing_quote_prefixes() -> None:
    ...
    aligned_result = format_pdf004(source, settings=settings_check.CheckSettings(select=("PDF004",), docstring_blank_line_style=settings_check.DocstringBlankLineStyle.ALIGNED))
...
def test_parenthesized_and_simple_suite_canonical_margins_are_used_for_aligned_blank_lines() -> None:
    ...
    settings = settings_check.CheckSettings(select=("PDF004",), docstring_blank_line_style=settings_check.DocstringBlankLineStyle.ALIGNED, indent_width=2)
    result = format_pdf004(source, settings=settings)
...
def test_aligned_style_preserves_tab_canonical_margin() -> None:
    ...
    settings = settings_check.CheckSettings(select=("PDF004",), docstring_blank_line_style=settings_check.DocstringBlankLineStyle.ALIGNED, indent_width=4)
    result = format_pdf004(source, settings=settings)
...
def test_check_fix_line_numbers_and_fix_false_findings_agree() -> None:
    ...
    _, context = contexts(source)
    findings = PDF004DocstringBlankLineWhitespace.check(context)
    fixed = PDF004DocstringBlankLineWhitespace.fix(context)
    ...
    assert PDF004DocstringBlankLineWhitespace.check(fixed_context) == ()
...
def test_pdf000_can_literalize_escaped_blank_line_before_pdf004_normalizes_it() -> None:
    source = 'def function():\n    """Summary.\\n   \\nBody."""\n'
    settings = settings_check.CheckSettings(select=("PDF000", "PDF004"))
