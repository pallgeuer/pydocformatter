...
from pydocformatter.rules.definitions.PDF.PDF403_section_underline_format import PDF403SectionUnderlineFormat
...
def format_source(source: str, *, settings: CheckSettings | None = None) -> formatter.FormatterResult:
    ...
    resolved_settings = CheckSettings(select=("PDF403",), docstring_convention=DocstringConvention.NUMPY) if settings is None else settings
...
def test_replaces_malformed_numpy_section_underline() -> None:
    ...
    assert result.fixed_findings[PDF403SectionUnderlineFormat.meta] == 1
    assert PDF403SectionUnderlineFormat.meta.name == "section-underline-format"
...
def test_ignored_outside_numpy_convention() -> None:
    source = 'def function(value):\n    """Summary.\n\n    Parameters\n    ===\n    value : int\n        Description.\n    """\n'
    result = format_source(source, settings=CheckSettings(select=("PDF403",), docstring_convention=DocstringConvention.GOOGLE))
