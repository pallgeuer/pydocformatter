...
def test_tab_indent_style_preserves_plain_docstring_tab_indentation() -> None:
    source = 'class Example:\n\tdef method(self):\n\t\t"""Summary.\n\t\tBody.\n\t\t"""\n'
    result = format_pdf002(source, settings=CheckSettings(select=("PDF100",), indent_style=IndentStyle.TAB))
...
def test_google_sections_use_canonical_header_entry_and_continuation_indentation() -> None:
    ...
    result = format_pdf002(source, settings=CheckSettings(select=("PDF100",), docstring_convention=DocstringConvention.GOOGLE))
...
def test_numpy_section_on_first_docstring_line_keeps_header_adornment_and_entries_aligned() -> None:
    source = 'def function(value):\n    """Parameters\n      ----------\n        value : int\n            Description.\n    """\n'
    result = format_pdf002(source, settings=CheckSettings(select=("PDF100",), docstring_convention=DocstringConvention.NUMPY))
...
def test_none_and_pep257_treat_convention_looking_text_as_ordinary_content() -> None:
    ...
    none_result = format_pdf002(source, settings=CheckSettings(select=("PDF100",), docstring_convention=DocstringConvention.NONE))
    pep257_result = format_pdf002(source, settings=CheckSettings(select=("PDF100",), docstring_convention=DocstringConvention.PEP257))
...
def test_simple_statement_suite_uses_one_configured_indent_unit_after_line_indent() -> None:
    ...
    result = format_pdf002(source, settings=CheckSettings(select=("PDF100",), indent_width=2))
...
def test_already_canonical_multiline_docstring_has_no_fix_or_finding() -> None:
    ...
    result = PDF100IncorrectIndentation.fix(context)

    assert PDF100IncorrectIndentation.check(context) == ()
...
def test_check_fix_line_numbers_and_fix_false_findings_agree() -> None:
    source = 'def first():\n    """Summary.\n      Over.\n    """\n\ndef second():\n    """Summary.\n      Also over.\n    """\n'
    _, context = contexts(source)
    findings = PDF100IncorrectIndentation.check(context)
    fixed = PDF100IncorrectIndentation.fix(context)
    ...
    assert PDF100IncorrectIndentation.check(fixed_context) == ()
...
def test_one_docstring_reports_all_changed_physical_lines_together() -> None:
    ...
    assert tuple(finding.line_numbers for finding in PDF100IncorrectIndentation.check(context)) == ((3, 4),)
