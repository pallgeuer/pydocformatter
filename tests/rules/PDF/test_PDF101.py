...
def test_disabled_structure_parsing_prevents_summary_separator_insertion() -> None:
    source = 'def function():\n    """Summary.\n    ```text\n    value\n    ```\n    """\n'
    settings = CheckSettings(select=("PDF101",), docstring_parse_code_fences=False)
    result = format_source(source, settings=settings)
...
def test_google_section_syntax_is_not_separated_without_google_convention(convention: DocstringConvention) -> None:
    source = 'def function(value):\n    """Summary.\n    Args:\n        value: Description.\n    """\n'
    settings = CheckSettings(select=("PDF101",), docstring_convention=convention)
    result = format_source(source, settings=settings)
...
def test_numpy_section_syntax_is_separated_as_heading_without_numpy_convention(convention: DocstringConvention) -> None:
    source = 'def function(value):\n    """Summary.\n    Parameters\n    ----------\n    value : int\n        Description.\n    """\n'
    settings = CheckSettings(select=("PDF101",), docstring_convention=convention, docstring_blank_line_after_last_section=True)
    result = format_source(source, settings=settings)
...
def test_final_section_setting_does_not_add_trailing_blank_after_non_section_structure() -> None:
    source = 'def function():\n    """Summary.\n\n    - item\n      detail\n    """\n'
    settings = CheckSettings(select=("PDF101",), docstring_convention=DocstringConvention.GOOGLE, docstring_blank_line_after_last_section=True)
    result = format_source(source, settings=settings)
...
def test_inserts_blank_line_between_summary_and_numpy_section() -> None:
    source = 'def function(value):\n    """Summary.\n    Parameters\n    ----------\n    value : int\n        Description.\n    """\n'
    settings = CheckSettings(select=("PDF101",), docstring_convention=DocstringConvention.NUMPY)
    result = format_source(source, settings=settings)
...
def test_parenthesized_docstring_uses_quote_column_margin_for_aligned_insertions() -> None:
    source = 'def function(value):\n    (\n        """Summary.\n        Args:\n            value: Description.\n        """\n    )\n'
    settings = CheckSettings(select=("PDF101",), docstring_convention=DocstringConvention.GOOGLE, docstring_blank_line_style=DocstringBlankLineStyle.ALIGNED)
    result = format_source(source, settings=settings)
...
def test_multiple_missing_blank_lines_are_inserted_in_one_docstring() -> None:
    ...
    findings = PDF101MissingBlankLine.check(context)
    result = format_source(source, settings=settings)
...
def test_final_section_setting_does_not_apply_without_matching_convention() -> None:
    source = 'def function(value):\n    """Summary.\n\n    Args:\n        value: Description."""\n'
    settings = CheckSettings(select=("PDF101",), docstring_convention=DocstringConvention.NONE, docstring_blank_line_after_last_section=True)
    result = format_source(source, settings=settings)
...
def test_preserves_crlf_for_generated_blank_lines() -> None:
    source = 'def function(value):\r\n    """Summary.\r\n    Args:\r\n        value: Description.\r\n    """\r\n'
    settings = CheckSettings(select=("PDF101",), docstring_convention=DocstringConvention.GOOGLE, line_ending=LineEnding.CR_LF)
    result = format_source(source, settings=settings)
...
def test_simple_suite_tab_margin_is_used_for_aligned_insertions() -> None:
    ...
    settings = CheckSettings(select=("PDF101",), docstring_convention=DocstringConvention.GOOGLE, docstring_blank_line_style=DocstringBlankLineStyle.ALIGNED, indent_style=IndentStyle.TAB)
    result = format_source(source, settings=settings)
...
def test_check_fix_and_fix_false_findings_agree() -> None:
    ...
    _, context = contexts(source, settings=settings)
    findings = PDF101MissingBlankLine.check(context)
    fixed = PDF101MissingBlankLine.fix(context)
    ...
    assert PDF101MissingBlankLine.check(fixed_context) == ()
...
def test_pdf100_and_pdf101_converge_with_configured_final_section_blank() -> None:
    source = 'def function(value):\n    """Summary.\n    Args:\n        value: Description.\n\n\n    """\n'
    settings = CheckSettings(select=("PDF100", "PDF101"), docstring_convention=DocstringConvention.GOOGLE, docstring_blank_line_after_last_section=True)
    result = format_source(source, settings=settings)
...
def test_pdf100_and_pdf101_converge_with_default_final_section_blank_disabled() -> None:
    source = 'def function(value):\n    """Summary.\n    Args:\n        value: Description.\n\n\n    """\n'
    settings = CheckSettings(select=("PDF100", "PDF101"), docstring_convention=DocstringConvention.GOOGLE)
    result = format_source(source, settings=settings)
