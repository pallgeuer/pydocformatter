...
from pydocformatter.rules.definitions.PDF.PDF407_convention_entry_spacing import PDF407ConventionEntrySpacing
...
def test_normalizes_google_entry_spacing() -> None:
    ...
    assert result.fixed_findings[PDF407ConventionEntrySpacing.meta] == 1
...
def test_normalizes_numpy_entry_spacing() -> None:
    ...
    settings = CheckSettings(select=("PDF407",), docstring_convention=DocstringConvention.NUMPY)
...
def test_normalizes_rest_field_spacing() -> None:
    ...
    settings = CheckSettings(select=("PDF407",), docstring_convention=DocstringConvention.REST)
    result = format_source(source, settings=settings)
...
def test_literal_block_parsing_setting_controls_entry_spacing_inside_literal_blocks() -> None:
    ...
    unprotected_settings = CheckSettings(select=("PDF407",), docstring_convention=DocstringConvention.GOOGLE, docstring_parse_literal_blocks=False)
    ...
    assert unprotected.fixed_findings[PDF407ConventionEntrySpacing.meta] == 1
