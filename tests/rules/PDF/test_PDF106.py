...
from pydocformatter.rules.definitions.PDF.PDF106_docstring_should_be_one_line import PDF106DocstringShouldBeOneLine
...
def contexts(source: str, *, settings: CheckSettings | None = None) -> tuple[RuleCategoryContext, RuleContext]:
    ...
    category = RuleCategoryContext(
        path="example.py",
        settings=CheckSettings(select=("PDF106",)) if settings is None else settings,
        module=module,
...
def test_collapses_single_summary_line_docstring() -> None:
    ...
    assert result.fixed_findings[PDF106DocstringShouldBeOneLine.meta] == 1
...
def test_collapses_module_docstring_and_preserves_closing_line_suffix() -> None:
    ...
    assert result.fixed_findings[PDF106DocstringShouldBeOneLine.meta] == 2
    assert not format_pdf106(result.new_source).modified
...
def test_line_length_boundary_includes_complete_source_line() -> None:
    ...
    fits = format_pdf106(source, settings=CheckSettings(select=("PDF106",), line_length=len('    """Summary."""')))
    too_long = format_pdf106(source, settings=CheckSettings(select=("PDF106",), line_length=len('    """Summary."""') - 1))

    assert fits.new_source == 'def function():\n    """Summary."""\n'
    assert fits.fixed_findings[PDF106DocstringShouldBeOneLine.meta] == 1
...
def test_line_length_includes_simple_suite_suffix() -> None:
    ...
    fits = format_pdf106(source, settings=CheckSettings(select=("PDF106",), line_length=len('def function(): """Summary."""; return None')))
    too_long = format_pdf106(source, settings=CheckSettings(select=("PDF106",), line_length=len('def function(): """Summary."""; return None') - 1))
...
def test_line_length_includes_parenthesized_prefix_and_tab_width() -> None:
    ...
    parenthesized_too_long = format_pdf106(parenthesized, settings=CheckSettings(select=("PDF106",), line_length=len('    """Summary."""')))
    tab_width_four = format_pdf106(tabs, settings=CheckSettings(select=("PDF106",), line_length=len('    """Summary."""'), indent_width=4))
    tab_width_eight = format_pdf106(tabs, settings=CheckSettings(select=("PDF106",), line_length=len('    """Summary."""'), indent_width=8))
    ...
    assert tab_width_four.fixed_findings[PDF106DocstringShouldBeOneLine.meta] == 1
...
def test_escapes_quote_collisions_before_using_separator_fallback() -> None:
    ...
    assert result.fixed_findings[PDF106DocstringShouldBeOneLine.meta] == 3
    assert not format_pdf106(result.new_source).modified
...
def test_skips_docstrings_that_are_not_single_line_summaries() -> None:
    ...
    result = format_pdf106(source, settings=CheckSettings(select=("PDF106",), docstring_convention=DocstringConvention.GOOGLE))
...
def test_disabled_structure_parsing_can_make_structural_line_collapsible() -> None:
    ...
    disabled = format_pdf106(source, settings=CheckSettings(select=("PDF106",), docstring_parse_list_items=False))

    assert parsed.new_source == source
    assert not parsed.fixed_findings
    assert disabled.new_source == 'def function():\n    """- item"""\n'
    assert disabled.fixed_findings[PDF106DocstringShouldBeOneLine.meta] == 1


@pytest.mark.parametrize(
    ("settings", "content", "expected"),
    (
        (CheckSettings(select=("PDF106",), docstring_parse_headings=False), "# Heading", 'def function():\n    """# Heading"""\n'),
        (CheckSettings(select=("PDF106",), docstring_parse_directives=False), ".. note:: Title", 'def function():\n    """.. note:: Title"""\n'),
        (CheckSettings(select=("PDF106",), docstring_parse_sphinx_fields=False), ":param value: Description.", 'def function():\n    """:param value: Description."""\n'),
        (CheckSettings(select=("PDF106",), docstring_parse_block_quotes=False), "> quote", 'def function():\n    """> quote"""\n'),
        (CheckSettings(select=("PDF106",), docstring_parse_doctests=False, docstring_parse_block_quotes=False), ">>> call()", 'def function():\n    """>>> call()"""\n'),
    ),
)
...
def test_pdf001_can_reflow_multiline_summary_before_pdf106_collapses_it() -> None:
    source = 'def function():\n    """Summary line\n    continuation line.\n    """\n'
    settings = CheckSettings(select=("PDF001", "PDF106"), line_length=88)
...
def test_check_fix_line_numbers_and_fix_false_findings_agree() -> None:
    ...
    findings = PDF106DocstringShouldBeOneLine.check(context)
    fixed = PDF106DocstringShouldBeOneLine.fix(context)
    ...
    assert PDF106DocstringShouldBeOneLine.check(fixed_context) == ()
