...
from pydocformatter.rules.definitions.PDF.PDF401_section_name_trailing_content import PDF401SectionNameTrailingContent
...
def test_moves_google_section_trailing_content_to_next_line() -> None:
    ...
    assert result.fixed_findings[PDF401SectionNameTrailingContent.meta] == 1
    assert PDF401SectionNameTrailingContent.meta.name == "section-name-trailing-content"
...
def test_uses_configured_indentation_when_moving_trailing_content() -> None:
    source = 'def function(value):\n\t"""Summary.\n\n\tArgs: value: Description.\n\t"""\n'
    settings = CheckSettings(select=("PDF401",), docstring_convention=DocstringConvention.GOOGLE, indent_style=IndentStyle.TAB)
...
def test_preserves_configured_crlf_line_endings_when_moving_trailing_content() -> None:
    source = 'def function(value):\r\n    """Summary.\r\n\r\n    Args: value: Description.\r\n    """\r\n'
    settings = CheckSettings(select=("PDF401",), docstring_convention=DocstringConvention.GOOGLE, line_ending=LineEnding.CR_LF)
    result = format_source(source, settings=settings)
...
def test_ignored_outside_google_convention() -> None:
    source = 'def function(value):\n    """Summary.\n\n    Args: value: Description.\n    """\n'
    result = format_source(source, settings=CheckSettings(select=("PDF401",), docstring_convention=DocstringConvention.NUMPY))
