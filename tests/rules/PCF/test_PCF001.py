...
def test_standalone_joining_is_opt_in() -> None:
    ...
    result = conftest.format_pcf(source, line_length=35, comment_join_standalone_lines=True)
...
def test_standalone_normalization_preserves_additional_hashes_and_eof_state() -> None:
    result = conftest.format_pcf("##Heading")
...
def test_list_items_and_block_quotes_reflow_with_stable_prefixes() -> None:
    ...
    result = conftest.format_pcf(source, line_length=34, comment_format_list_items=True, comment_format_block_quotes=True)
...
def test_ordered_nested_list_item_consumes_more_indented_continuation_lines() -> None:
    ...
    result = conftest.format_pcf(source, line_length=38, comment_format_list_items=True)
...
@pytest.mark.parametrize(
    ("source", "settings"),
    (
        ("## Heading\n# ordinary prose\n", CheckSettings(experimental=True, select=("PCF",), line_length=20, comment_preserve_headings=True)),
        ("# >>> value = function()\n# expected output\n", CheckSettings(experimental=True, select=("PCF",), line_length=20, comment_preserve_doctests=True)),
        ("# ```python\n# value = function()\n# ```\n", CheckSettings(experimental=True, select=("PCF",), line_length=20, comment_preserve_code_fences=True)),
        ("# Name | Value\n# ---- | -----\n# one  | two\n", CheckSettings(experimental=True, select=("PCF",), line_length=20, comment_preserve_tables=True)),
        ("# .. note::\n#    preserved directive body\n# ordinary prose\n", CheckSettings(experimental=True, select=("PCF",), line_length=20, comment_preserve_directives=True)),
    ),
)
def test_enabled_structure_detectors_preserve_recognized_regions(source: str, settings: CheckSettings) -> None:
    result = conftest.format_pcf_settings(source, settings=settings)
...
def test_preserved_structure_code_does_not_prevent_adjacent_prose_formatting() -> None:
    ...
    result = conftest.format_pcf(source, line_length=40, comment_preserve_code_fences=True, comment_detect_statements=True)
...
def test_restructuredtext_grid_and_simple_tables_are_preserved() -> None:
    ...
    result = conftest.format_pcf(source, line_length=12, comment_preserve_tables=True)
...
def test_disabled_code_detection_protects_the_whole_physical_run() -> None:
    ...
    result = conftest.format_pcf(source, line_length=35, comment_detect_code=True)
...
@pytest.mark.parametrize(
    ...
    (
        (
            ...
            CheckSettings(experimental=True, select=("PCF",), line_length=30, comment_detect_code=False, comment_detect_statements=True),
        ),
        (
            "# package.function(value)\n# prose that would otherwise wrap onto another line\n",
            CheckSettings(experimental=True, select=("PCF",), line_length=30, comment_detect_code=False, comment_detect_expressions=True),
        ),
    ),
)
def test_enabled_ast_code_detection_protects_the_whole_run(source: str, settings: CheckSettings) -> None:
    disabled = conftest.format_pcf(source, line_length=30, comment_detect_code=False, comment_detect_statements=False, comment_detect_expressions=False)
    enabled = conftest.format_pcf_settings(source, settings=settings)
...
def test_comment_edits_preserve_untouched_mixed_endings_and_use_configured_generated_endings() -> None:
    ...
    result = conftest.format_pcf(source, line_length=42, line_ending=LineEnding.LF)
...
def test_standalone_spacing_long_tokens_hash_boundaries_and_eof_are_stable(source: str, line_length: int, expected: str) -> None:
    assert conftest.format_pcf(source, line_length=line_length).new_source == expected


def test_indented_wrapping_accounts_for_comment_prefix_width() -> None:
    source = "if enabled:\n    # alpha beta gamma delta epsilon\n    pass\n"
    result = conftest.format_pcf(source, line_length=18)
...
def test_joining_does_not_cross_code_blank_hash_only_protected_or_indentation_boundaries() -> None:
    source = "# first line\nvalue = 1\n# second line\n\n# third line\n# noqa\n# fourth line\n###\n# fifth line\nif value:\n    # sixth line\n# seventh line\n"
    result = conftest.format_pcf(source, line_length=80, comment_join_standalone_lines=True)
    assert result.new_source == source
...
def test_list_formatting_can_be_disabled_and_then_uses_plain_wrapping() -> None:
    ...
    result = conftest.format_pcf(source, line_length=16, comment_format_list_items=False)
...
def test_tabbed_list_prefix_is_expanded_at_the_actual_comment_column() -> None:
    ...
    result = conftest.format_pcf(source, line_length=20, indent_width=4)
    assert result.new_source == "if enabled:\n\t#   - alpha beta\n\t#     gamma\n\t#     delta\n\tpass\n"


def test_block_quotes_join_only_identical_prefixes_and_preserve_nested_prefixes() -> None:
    source = "# > alpha beta\n# > gamma delta\n# >> nested alpha\n# >> nested beta\n# > final quote\n"
    result = conftest.format_pcf(source, line_length=22)
...
def test_block_quote_formatting_can_be_disabled_and_then_uses_plain_wrapping() -> None:
    source = "# > alpha beta gamma delta\n"
    result = conftest.format_pcf(source, line_length=17, comment_format_block_quotes=False)
    assert result.new_source == "# > alpha beta\n# gamma delta\n"
...
def test_heading_variants_are_preserved(source: str) -> None:
    assert conftest.format_pcf(source, line_length=8, comment_preserve_headings=True).new_source == source


def test_heading_preservation_can_be_disabled_without_misclassifying_short_adornments() -> None:
    source = "## Long heading words\n# title\n# --\n"
    result = conftest.format_pcf(source, line_length=14, comment_preserve_headings=False, comment_detect_statements=False)
...
def test_doctest_preservation_starts_at_first_prompt_and_ends_at_run_boundary() -> None:
    ...
    result = conftest.format_pcf(source, line_length=26)
    assert (
...
def test_fence_matching_handles_longer_closers_mismatched_markers_and_unclosed_fences(source: str, expected: str) -> None:
    assert conftest.format_pcf(source, line_length=20, comment_detect_statements=False).new_source == expected


@pytest.mark.parametrize("fence", ("```", "~~~"))
def test_fence_like_lines_with_trailing_text_do_not_close_preserved_regions(fence: str) -> None:
    source = f"# {fence}python\n# {fence}not-a-close\n# alpha beta gamma delta epsilon\n# {fence}\n"
    result = conftest.format_pcf(source, line_length=18, comment_preserve_code_fences=True, comment_preserve_headings=False, comment_detect_statements=False)
    assert result.new_source == source
...
def test_code_fence_preservation_can_be_disabled() -> None:
    ...
    result = conftest.format_pcf(source, line_length=18, comment_preserve_code_fences=False, comment_preserve_headings=False, comment_detect_statements=False)
...
def test_table_detection_requires_structure_and_does_not_preserve_arbitrary_pipes_or_dashes() -> None:
    ...
    result = conftest.format_pcf(source, line_length=24, comment_detect_statements=False)
...
def test_table_preservation_can_be_disabled() -> None:
    ...
    result = conftest.format_pcf(source, line_length=18, comment_preserve_tables=False, comment_detect_statements=False)
...
def test_directive_preservation_includes_only_more_indented_body_and_resumes_formatting() -> None:
    ...
    result = conftest.format_pcf(source, line_length=25)
...
def test_directive_preservation_can_be_disabled() -> None:
    ...
    result = conftest.format_pcf(source, line_length=20, comment_preserve_directives=False, comment_detect_statements=False)
    assert result.new_source == "# .. note::\n# ordinary directive\n# body words\n"
...
def test_disabled_code_heuristic_recognizes_all_documented_forms(code: str) -> None:
    ...
    assert conftest.format_pcf(source, line_length=24, comment_detect_code=True, comment_detect_statements=False).new_source == source
...
def test_disabled_code_heuristic_requires_keyword_boundaries(prose: str) -> None:
    source = f"# {prose}\n"
    assert conftest.format_pcf(source, line_length=20, comment_detect_code=True, comment_detect_statements=False).new_source != source
...
def test_statement_detection_recognizes_single_and_multiline_python(statement: str) -> None:
    ...
    assert conftest.format_pcf(source, line_length=20, comment_detect_statements=True).new_source == source


def test_statement_detection_parses_a_multiline_candidate_when_individual_lines_are_incomplete() -> None:
    source = "# value = (\n#     compute()\n# )\n"
    assert conftest.format_pcf(source, line_length=10, comment_detect_statements=True).new_source == source
...
def test_expression_detection_recognizes_nontrivial_expressions(expression: str) -> None:
    source = f"# {expression}\n# prose that otherwise wraps\n"
    assert conftest.format_pcf(source, line_length=20, comment_detect_statements=False, comment_detect_expressions=True).new_source == source
...
def test_code_detection_strips_enabled_list_and_quote_prefixes_and_protects_the_whole_run() -> None:
    ...
    statements = conftest.format_pcf(source, line_length=20, comment_detect_statements=True, comment_detect_expressions=False)
    expressions = conftest.format_pcf(source, line_length=20, comment_detect_statements=False, comment_detect_expressions=True)
...
def test_preserved_regions_split_multiline_code_candidates_without_hiding_adjacent_formatting() -> None:
    ...
    result = conftest.format_pcf(source, line_length=24, comment_detect_statements=True)
...
def test_complex_mixed_run_formats_each_semantic_unit_without_crossing_boundaries() -> None:
    ...
    result = conftest.format_pcf(source, line_length=34, comment_join_standalone_lines=True)
    assert (
        result.new_source
...
def test_standalone_rule_is_independent_of_trailing_rule_selection() -> None:
    ...
    settings = CheckSettings(experimental=True, select=("PCF001",))
    result = conftest.format_pcf_settings(source, settings=settings)
...
def test_read_only_check_reports_multiline_semantic_units_without_changing_source() -> None:
    ...
    checked = conftest.format_pcf(source, fix=False, line_length=24, comment_join_standalone_lines=True)
...
def test_tab_equivalent_block_quote_prefixes_normalize_as_one_finding() -> None:
    source = "# >\t> alpha beta\n# >\t> gamma delta\n"
    result = conftest.format_pcf(source, line_length=30, indent_width=4)
...
def test_empty_list_markers_and_quotes_are_not_mistaken_for_nonempty_structured_text() -> None:
    source = "# -\n# 1.\n# >\n# ordinary words requiring wrapping\n"
    result = conftest.format_pcf(source, line_length=20, comment_detect_statements=False)
...
def test_disabling_structure_formatting_also_disables_structure_prefix_stripping_for_code_detection() -> None:
    ...
    result = conftest.format_pcf(source, line_length=20, comment_format_list_items=False, comment_format_block_quotes=False, comment_detect_statements=True)
...
def test_multiline_expression_detection_requires_the_complete_segment_to_parse() -> None:
    ...
    protected = conftest.format_pcf(code_only, line_length=12, comment_detect_statements=False, comment_detect_expressions=True)
    formatted = conftest.format_pcf(mixed, line_length=18, comment_detect_statements=False, comment_detect_expressions=True)
...
def test_overlapping_preserved_structures_do_not_extend_past_their_combined_region() -> None:
    ...
    result = conftest.format_pcf(source, line_length=24, comment_preserve_code_fences=True, comment_preserve_doctests=False, comment_detect_statements=True)
...
def test_directive_with_tab_indented_body_at_end_of_run_is_preserved() -> None:
    ...
    result = conftest.format_pcf(source, line_length=12, indent_width=4)
    assert result.new_source == source


def test_unicode_code_points_each_count_as_one_width_column() -> None:
    wide = conftest.format_pcf("# 表表表 alpha beta\n", line_length=12, comment_detect_statements=False)
    combining = conftest.format_pcf("# e\u0301e\u0301e\u0301 alpha beta\n", line_length=12, comment_detect_statements=False)
...
def test_complex_standalone_formatting_is_idempotent(source: str, line_length: int, join_lines: bool, indent_width: int) -> None:
    first = conftest.format_pcf(source, line_length=line_length, comment_join_standalone_lines=join_lines, indent_width=indent_width)
    assert first.new_source is not None
    second = conftest.format_pcf(first.new_source, line_length=line_length, comment_join_standalone_lines=join_lines, indent_width=indent_width)
