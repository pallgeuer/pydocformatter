...
from pydocformatter.rules.definitions.PCF.PCF002_trailing_comment_extraction import PCF002TrailingCommentExtraction


def test_long_trailing_comment_moves_above_code_and_is_independent_of_pcf000() -> None:
...
def test_multiple_trailing_comment_edits_are_applied_in_source_order() -> None:
    ...
    assert {rule.code.tag: count for rule, count in result.fixed_findings.items()} == {"PCF001": 3, "PCF002": 2}
...
def test_protected_directives_are_not_extracted_by_pcf002(directive: str) -> None:
...
def test_unicode_and_namespaced_directives_are_unsafe_to_extract_but_glued_argument_is_not_a_directive() -> None:
    ...
    settings = CheckSettings(select=("PCF002",), line_length=60)
...
def test_syntax_aware_extraction_still_normalizes_spacing_without_moving_sensitive_comment() -> None:
    ...
    assert {rule.code.tag: count for rule, count in result.fixed_findings.items()} == {"PCF001": 1}
...
def test_task_marker_trailing_comment_extracts_with_hanging_indentation() -> None:
    ...
    assert {rule.code.tag: count for rule, count in result.fixed_findings.items()} == {"PCF001": 1, "PCF002": 1}
...
def test_spacing_and_extraction_both_report_same_original_line_in_check_mode() -> None:
    ...
    settings = CheckSettings(select=("PCF001", "PCF002"), line_length=28)
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=False)
    assert result.new_source == source
    assert tuple((finding.rule.code.tag, finding.line_numbers) for finding in result.unfixed_findings) == (("PCF001", (1,)), ("PCF002", (1,)))
...
def test_pcf_rule_selection_keeps_actions_separate() -> None:
    ...
    standalone_settings = CheckSettings(select=("PCF000",), line_length=28)
    spacing_settings = CheckSettings(select=("PCF001",), line_length=28)
    directive_settings = CheckSettings(select=("PCF100",), line_length=28)
    extraction_settings = CheckSettings(select=("PCF002",), line_length=28)
    spacing_extraction_directive_settings = CheckSettings(select=("PCF001", "PCF100", "PCF002"), line_length=28)
...
def test_content_detection_keeps_indented_disabled_code_inline_when_spacing_is_unselected() -> None:
    ...
    settings = CheckSettings(select=("PCF002",), line_length=24, comment_detect_code=True)
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
    assert result.new_source == source
...
def test_direct_extraction_keeps_recognized_markup_indivisible() -> None:
    ...
    settings = CheckSettings(select=("PCF002",), line_length=24, url_aware_wrapping=False)
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
    ...
    assert result.fixed_findings[PCF002TrailingCommentExtraction.meta] == 1
...
def test_fence_opener_with_info_string_remains_fixable_during_extraction() -> None:
    ...
    settings = CheckSettings(select=("PCF002",), line_length=28, comment_trailing_extraction_content_aware=False)
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
...
def test_extraction_wraps_existing_scan_without_calling_text_wrapper(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = CheckSettings(select=("PCF002",), line_length=28)
...
def test_ambiguous_trailing_markup_is_reported_without_extraction(content_aware: bool) -> None:
    ...
    settings = CheckSettings(select=("PCF002",), line_length=28, comment_trailing_extraction_content_aware=content_aware)
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
...
def test_ambiguity_guard_precedes_content_and_task_marker_handling() -> None:
    ...
    for source in sources:
        settings = CheckSettings(select=("PCF002",), line_length=28, comment_task_marker_mode=CommentTaskMarkerMode.NO_WRAP)
        result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
...
def test_ambiguity_is_not_reported_before_overlong_and_syntax_eligibility() -> None:
    ...
    short_result = formatter.format_source(
        short, "example.py", settings=CheckSettings(select=("PCF002",), line_length=80), rule_selection=rules_selection.select_rules(CheckSettings(select=("PCF002",), line_length=80)), fix=True
    )
    sensitive_settings = CheckSettings(select=("PCF002",), line_length=25)
...
def test_inline_link_destinations_activate_extraction_url_balancing() -> None:
    ...
    disabled_settings = CheckSettings(select=("PCF002",), line_length=40, url_aware_wrapping=False)
    enabled_settings = CheckSettings(select=("PCF002",), line_length=40, url_aware_wrapping=True)
...
def test_unicode_barrier_is_ignored_below_limit_and_diagnostic_when_extraction_is_needed() -> None:
    ...
    short_settings = CheckSettings(select=("PCF002",), line_length=80)
    long_settings = CheckSettings(select=("PCF002",), line_length=32)
...
def test_unicode_barrier_width_uses_canonical_comment_spacing() -> None:
    ...
    extraction_settings = CheckSettings(select=("PCF002",), line_length=30)
    combined_settings = CheckSettings(select=("PCF001", "PCF002"), line_length=30)
    ...
    assert tuple(finding.rule.code.tag for finding in checked.unfixed_findings) == ("PCF001",)
    assert fixed.new_source == "value = 1  # hazard\u202e\n"
    assert {rule.code.tag: count for rule, count in fixed.fixed_findings.items()} == {"PCF001": 1}
...
def test_unicode_barrier_width_uses_configured_tab_width(indent_width: int, expected_finding: bool) -> None:
    source = "if enabled:\n\tvalue = 1  # hazard\u202e\n"
    settings = CheckSettings(select=("PCF002",), line_length=26, indent_width=indent_width)
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
...
def test_unicode_barrier_honors_syntax_aware_extraction() -> None:
    ...
    settings = CheckSettings(select=("PCF002",), line_length=32, comment_trailing_extraction_syntax_aware=True)
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
...
def test_unicode_barrier_remains_diagnostic_before_content_aware_extraction() -> None:
    ...
    settings = CheckSettings(select=("PCF002",), line_length=32, comment_trailing_extraction_content_aware=True)
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
