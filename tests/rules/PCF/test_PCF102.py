"""Tests for PCF102 rule-codes-in-suppression-comments."""
...
from pydocformatter.rules.definitions.PCF.PCF102_rule_codes_in_suppression_comments import PCF102RuleCodesInSuppressionComments
...
def format_pcf102(source: str, *, fix: bool = True, extra_select: tuple[str, ...] = (), unfixable: tuple[str, ...] = ()) -> formatter.FormatterResult:
    settings = CheckSettings(select=("PCF102", *extra_select), unfixable=unfixable)
...
def test_fixes_all_exact_pydocfmt_codes_in_one_directive() -> None:
    source = "# pydocfmt: ignore[PDF101, PCF000]  # reason\nvalue = 1\n"

    checked = format_pcf102(source, fix=False)
    fixed = format_pcf102(source)
    ...
    assert fixed.fixed_findings[PCF102RuleCodesInSuppressionComments.meta] == 1


def test_semantic_aliases_retain_the_first_source_occurrence() -> None:
    code_first = format_pcf102("# pydocfmt: ignore[PDF101, docstring-reflow]\nvalue = 1\n")
    name_first = format_pcf102("# pydocfmt: ignore[docstring-reflow, PDF101]\nvalue = 1\n")
...
def test_permits_broad_pydocfmt_selectors_and_ignores_unknown_or_invalid_tokens() -> None:
    ...
    result = format_pcf102(source, fix=False)
...
def test_reports_unknown_and_invalid_tokens_only_through_pcf101() -> None:
    ...
    result = format_pcf102(source, fix=False, extra_select=("PCF101",))
...
def test_configured_unfixable_selection_retains_the_local_policy_finding_in_fix_mode() -> None:
    ...
    result = format_pcf102(source, unfixable=("PCF102",))

    assert result.new_source == source
    assert tuple((finding.rule, finding.fixable) for finding in result.unfixed_findings) == ((PCF102RuleCodesInSuppressionComments.meta, False),)
...
def test_rule_metadata_and_default_explicit_selection() -> None:
    ...
    code_selection = rules_selection.select_rules(CheckSettings(select=("PCF102",)))
    name_selection = rules_selection.select_rules(CheckSettings(select=("rule-codes-in-suppression-comments",)))
    reduced_require_explicit = tuple(selector for selector in CheckSettings().require_explicit if selector != "PCF102")
    broad_selection = rules_selection.select_rules(CheckSettings(select=("PCF",), require_explicit=reduced_require_explicit))

    assert PCF102RuleCodesInSuppressionComments.meta.fix_availability is FixAvailability.SOMETIMES
    assert PCF102RuleCodesInSuppressionComments.meta not in tuple(rule.rule for rule in default_selection.rules)
    assert PCF102RuleCodesInSuppressionComments.meta not in tuple(rule.rule for rule in category_selection.rules)
    assert PCF102RuleCodesInSuppressionComments.meta in tuple(rule.rule for rule in code_selection.rules)
    assert PCF102RuleCodesInSuppressionComments.meta in tuple(rule.rule for rule in name_selection.rules)
    assert PCF102RuleCodesInSuppressionComments.meta in tuple(rule.rule for rule in broad_selection.rules)
    assert broad_selection.errors == ()


def test_finding_can_be_suppressed_by_code_or_name() -> None:
    by_code_source = "# pydocfmt: ignore[PCF102, PDF101]\n"
    by_name_source = "# pydocfmt: ignore[rule-codes-in-suppression-comments, PDF101]\n"
    by_code = format_pcf102(by_code_source)
    by_name = format_pcf102(by_name_source)
...
def test_pcf100_then_policy_fix_and_pcf101_audit_converge_on_refreshed_source() -> None:
    ...
    result = format_pcf102(source, extra_select=("PCF100", "PDF101", "PCF101"))

    assert result.new_source == "# pydocfmt: ignore[docstring-reflow]\nvalue = 1\n"
    assert result.fixed_findings[PCF102RuleCodesInSuppressionComments.meta] == 1
    assert tuple((str(finding.rule.code), finding.message) for finding in result.unfixed_findings) == (("PCF101", "Suppression selector 'docstring-reflow' did not suppress any findings"),)
...
def test_policy_fix_preserves_unrelated_duplicates_unless_pcf100_is_selected() -> None:
    ...
    policy_only = format_pcf102(source)
    with_normalization = format_pcf102(source, extra_select=("PCF100",))
