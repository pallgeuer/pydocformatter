"""Tests for PCF009 rule-names-in-suppression-comments."""
...
from pydocformatter.rules.definitions.PCF.PCF009_rule_names_in_suppression_comments import PCF009RuleNamesInSuppressionComments
...
def format_pcf009(source: str, *, fix: bool = True, extra_select: tuple[str, ...] = (), unfixable: tuple[str, ...] = ()) -> formatter.FormatterResult:
    settings = CheckSettings(select=("PCF009", *extra_select), unfixable=unfixable)
...
def test_fixes_all_exact_pydocfmt_names_in_one_directive() -> None:
    ...
    checked = format_pcf009(source, fix=False)
    fixed = format_pcf009(source)

    assert tuple((finding.line_numbers, finding.fixable) for finding in checked.unfixed_findings) == (((1,), True),)
    assert fixed.new_source == "# pydocfmt: file-ignore[PDF101, PCF001]  # reason\nvalue = 1\n"
    assert fixed.fixed_findings[PCF009RuleNamesInSuppressionComments.meta] == 1


def test_semantic_aliases_retain_the_first_source_occurrence() -> None:
    name_first = format_pcf009("# pydocfmt: ignore[docstring-reflow, PDF101]\nvalue = 1\n")
    code_first = format_pcf009("# pydocfmt: ignore[PDF101, docstring-reflow]\nvalue = 1\n")
...
def test_leaves_broad_unknown_and_invalid_pydocfmt_tokens_alone() -> None:
    ...
    result = format_pcf009(source, fix=False)
...
def test_reports_unknown_and_invalid_tokens_only_through_pcf006() -> None:
    ...
    result = format_pcf009(source, fix=False, extra_select=("PCF006",))
...
def test_configured_unfixable_selection_retains_the_local_policy_finding_in_fix_mode() -> None:
    ...
    result = format_pcf009(source, unfixable=("PCF009",))

    assert result.new_source == source
    assert tuple((finding.rule, finding.fixable) for finding in result.unfixed_findings) == ((PCF009RuleNamesInSuppressionComments.meta, False),)
...
def test_rule_metadata_and_default_explicit_selection() -> None:
    ...
    code_selection = rules_selection.select_rules(CheckSettings(select=("PCF009",)))
    name_selection = rules_selection.select_rules(CheckSettings(select=("rule-names-in-suppression-comments",)))
    reduced_require_explicit = tuple(selector for selector in CheckSettings().require_explicit if selector != "PCF009")
    broad_selection = rules_selection.select_rules(CheckSettings(select=("PCF",), require_explicit=reduced_require_explicit))

    assert PCF009RuleNamesInSuppressionComments.meta.fix_availability is FixAvailability.SOMETIMES
    assert PCF009RuleNamesInSuppressionComments.meta not in tuple(rule.rule for rule in default_selection.rules)
    assert PCF009RuleNamesInSuppressionComments.meta not in tuple(rule.rule for rule in category_selection.rules)
    assert PCF009RuleNamesInSuppressionComments.meta in tuple(rule.rule for rule in code_selection.rules)
    assert PCF009RuleNamesInSuppressionComments.meta in tuple(rule.rule for rule in name_selection.rules)
    assert PCF009RuleNamesInSuppressionComments.meta in tuple(rule.rule for rule in broad_selection.rules)
    assert broad_selection.errors == ()


def test_finding_can_be_suppressed_by_code_or_name() -> None:
    by_code_source = "# pydocfmt: ignore[PCF009, docstring-reflow]\n"
    by_name_source = "# pydocfmt: ignore[rule-names-in-suppression-comments, docstring-reflow]\n"
    by_code = format_pcf009(by_code_source)
    by_name = format_pcf009(by_name_source)
...
def test_pcf003_then_policy_fix_and_pcf006_audit_converge_on_refreshed_source() -> None:
    ...
    result = format_pcf009(source, extra_select=("PCF003", "PDF101", "PCF006"))

    assert result.new_source == "# pydocfmt: ignore[PDF101]\nvalue = 1\n"
    assert result.fixed_findings[PCF009RuleNamesInSuppressionComments.meta] == 1
    assert tuple((str(finding.rule.code), finding.message) for finding in result.unfixed_findings) == (("PCF006", "Suppression selector 'PDF101' did not suppress any findings"),)
...
def test_policy_fix_preserves_unrelated_duplicates_unless_pcf003_is_selected() -> None:
    ...
    policy_only = format_pcf009(source)
    with_normalization = format_pcf009(source, extra_select=("PCF003",))
