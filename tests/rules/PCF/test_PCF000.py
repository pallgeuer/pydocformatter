...
from pydocformatter.rules.definitions.PCF.PCF000_standalone_comment_formatting import PCF000StandaloneCommentFormatting
...
@pytest.mark.parametrize("settings", [CheckSettings(select=("PCF000",)), CheckSettings()])
...
def test_formats_original_source_comment_after_libcst_normalized_empty_line(line_ending: str) -> None:
    ...
    settings = CheckSettings(select=("PCF000",), line_length=32)
    ...
    assert fixed.fixed_findings[PCF000StandaloneCommentFormatting.meta] == 1
...
def test_formats_multiline_fstring_and_later_standalone_comments_on_python_311() -> None:
    ...
    expected = 'x = f"""{1\n# inner\n}"""\n# Outer comment needs a space.\n'
    settings = CheckSettings(select=("PCF000",))
    ...
    assert fixed.new_source == expected
    assert fixed.fixed_findings[PCF000StandaloneCommentFormatting.meta] == 2
    assert not fixed.errors
    assert not repeated.modified
    assert not repeated.errors
...
def test_ascii_horizontal_whitespace_only_standalone_comments_become_bare_hash(payload: str) -> None:
    ...
    assert result.fixed_findings[PCF000StandaloneCommentFormatting.meta] == 1
...
def test_empty_comment_normalization_is_idempotent_and_does_not_overlap_pcf001() -> None:
    source = "#   \nvalue = 1  #   \n"
    first = pcf_helpers.format_pcf_settings(source, settings=CheckSettings(select=("PCF000", "PCF001")))
    assert first.new_source is not None
    second = pcf_helpers.format_pcf_settings(first.new_source, settings=CheckSettings(select=("PCF000", "PCF001")))
...
def test_ambiguous_markup_reports_without_rewriting_semantic_body() -> None:
    ...
    settings = CheckSettings(select=("PCF000",), line_length=28)
...
def test_ambiguous_markup_allows_marker_only_partial_fix_then_stabilizes() -> None:
    ...
    assert first.fixed_findings[PCF000StandaloneCommentFormatting.meta] == 1
    assert second.new_source == first.new_source
...
def test_ambiguous_joined_unit_marker_fix_preserves_every_body_and_mixed_line_ending() -> None:
    ...
    settings = CheckSettings(select=("PCF000",), line_length=20, comment_join_standalone_lines=True)
    first = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
    assert first.new_source is not None
    second = formatter.format_source(first.new_source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)
...
def test_unicode_barrier_preserves_marker_only_and_mixed_joined_comment_bodies() -> None:
    source = "#\u202e\n#Safe words.\n# Keep  unsafe\u2060words.\n#More safe words.\n"
    settings = CheckSettings(select=("PCF000",), comment_join_standalone_lines=True)
    result = formatter.format_source(source, "example.py", settings=settings, rule_selection=rules_selection.select_rules(settings), fix=True)

    assert result.new_source == "# \u202e\n# Safe words.\n# Keep  unsafe\u2060words.\n# More safe words.\n"
    assert result.fixed_findings[PCF000StandaloneCommentFormatting.meta] == 3


@pytest.mark.parametrize(
    ("source", "settings"),
    [
        ("#>>> value = function()\u200b\n#expected output\n", CheckSettings(select=("PCF000",), comment_preserve_doctests=True)),
        ("#```python\n#value = function()\u200b\n#```\n", CheckSettings(select=("PCF000",), comment_preserve_code_fences=True)),
        ("#Name | Value\n#---- | -----\n#one\u200b | two\n", CheckSettings(select=("PCF000",), comment_preserve_tables=True)),
        ("#.. note::\n#   preserved\u200b directive body\n# ordinary prose\n", CheckSettings(select=("PCF000",), comment_preserve_directives=True)),
    ],
)
