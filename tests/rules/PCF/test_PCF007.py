...
from pydocformatter.rules.definitions.PCF.PCF007_comment_suspicious_unicode import PCF007CommentSuspiciousUnicode
...
def format_pcf007(source: str, *, fix: bool = True) -> formatter.FormatterResult:
    settings = CheckSettings(select=("PCF007",))
...
def test_fixes_nonbreaking_comment_indentation(char: str) -> None:
    ...
    result = format_pcf007(source)

    assert result.new_source == "# Comment.\nvalue = 1  # Trailing.\n"
    assert result.fixed_findings[PCF007CommentSuspiciousUnicode.meta] == 2
...
def test_groups_repeated_characters_per_comment_but_not_across_comments() -> None:
    ...
    checked = format_pcf007(source, fix=False)
    fixed = format_pcf007(source)
    ...
    assert fixed.fixed_findings[PCF007CommentSuspiciousUnicode.meta] == 2
...
def test_whitespace_only_payload_remains_indentation_until_line_end() -> None:
    ...
    assert result.fixed_findings[PCF007CommentSuspiciousUnicode.meta] == 4
...
def test_nonbreaking_space_after_prose_is_not_reclassified_by_later_line_break_characters() -> None:
    ...
    assert result.fixed_findings[PCF007CommentSuspiciousUnicode.meta] == 1
...
def test_rule_is_selected_broadly_and_is_sometimes_fixable() -> None:
    ...
    assert PCF007CommentSuspiciousUnicode.meta in tuple(rule.rule for rule in default_selection.rules)
    selected = next(rule for rule in category_selection.rules if rule.rule == PCF007CommentSuspiciousUnicode.meta)
    assert selected.fixable


def test_unfixable_selection_reports_fixable_occurrences_without_editing() -> None:
    source = "#\u00a0Comment.\n"
    settings = CheckSettings(select=("PCF007",), unfixable=("PCF007",))
...
def test_local_suppression_hides_fixable_and_diagnostic_occurrences(payload: str) -> None:
    source = f"# pydocfmt: ignore[PCF007]\n# {payload}\n"
...
def test_comment_formatters_preserve_hazards_until_pcf007_reports_or_fixes_them() -> None:
    ...
    assert result.fixed_findings[PCF007CommentSuspiciousUnicode.meta] == 1
    assert tuple(finding.rule for finding in result.unfixed_findings).count(PCF007CommentSuspiciousUnicode.meta) == 3


def test_pcf002_and_pcf007_apply_overlapping_trailing_comment_fixes_convergently() -> None:
    source = "value = 1#\u00a0Comment.\n"
    settings = CheckSettings(select=("PCF002", "PCF007"))

    result = pcf_helpers.format_pcf_settings(source, settings=settings)

    assert result.new_source == "value = 1  # Comment.\n"
    assert {rule.code.tag: count for rule, count in result.fixed_findings.items()} == {"PCF002": 1, "PCF007": 1}
