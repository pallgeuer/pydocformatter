...
def test_find_git_root_for_path_returns_none_outside_git_root() -> None:
    with tempfile.TemporaryDirectory() as td:
        assert misc.find_git_root_for_path(td) is None


def test_find_git_root_for_path_uses_cached_intermediate_directory() -> None:
    with tempfile.TemporaryDirectory() as td:
        root = Path(td)
...
def test_find_git_root_for_path_backfills_visited_directories() -> None:
    with tempfile.TemporaryDirectory() as td:
        root = Path(td)
        nested = root / "src" / "pkg"
        nested.mkdir(parents=True)
