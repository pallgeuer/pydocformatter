...
class TestRules(unittest.TestCase):
    ...
    def test_import_package_rule_categories_validates_package_structure_and_registration(self) -> None:
        ...
        cases: tuple[tuple[str, dict[str, str], str], ...] = (
            ("stray definitions module", {**valid_files, "stray.py": ""}, "must contain only category packages"),
            ("nested category package", {**valid_files, "PDF/nested/__init__.py": ""}, "must not contain nested packages"),
            ("missing category module", {path: content for path, content in valid_files.items() if path != "PDF/PDF.py"}, "must contain category module"),
            ("wrong category class name", {**valid_files, "PDF/PDF.py": valid_files["PDF/PDF.py"].replace("class PDF", "class WrongPDF")}, "exactly one RuleCategoryBase subclass named PDF"),
            ("category metadata prefix mismatch", {**valid_files, "PDF/PDF.py": valid_files["PDF/PDF.py"].replace("prefix='PDF'", "prefix='PCF'")}, "does not match package and module name"),
            ("unregistered category", {**valid_files, "PDF/PDF.py": valid_files["PDF/PDF.py"].replace("@rule_registration.register_rule_category\n", "")}, "is not registered with the rule registry"),
            ("unexpected category module", {**valid_files, "PDF/helper.py": "value = 1\n"}, "Unexpected module in rule category package"),
            ("rule module without name suffix", {**valid_files, "PDF/PDF100.py": ""}, "Unexpected module in rule category package"),
            ("reserved rule module code", {**valid_files, "PDF/ALL001_test.py": ""}, "Unexpected module in rule category package"),
            (
            ...
            ("rule module code mismatch", {**valid_files, "PDF/PDF101_test.py": valid_files["PDF/PDF101_test.py"].replace("RuleCode('PDF101')", "RuleCode('PDF100')")}, "does not match rule code"),
            (
                "unregistered rule",
            ...
            ("missing category documentation", {path: content for path, content in valid_files.items() if path != "PDF/PDF.md"}, "missing adjacent documentation PDF.md"),
            ("missing rule documentation", {path: content for path, content in valid_files.items() if path != "PDF/PDF101_test.md"}, "missing adjacent documentation PDF101_test.md"),
            ("orphan documentation", {**valid_files, "PDF/PDF999_orphan.md": "# orphan\n"}, "contains orphan Markdown files"),
        )
        for name, files, message in cases:
            with self.subTest(name=name), pytest.raises(rule_registration.RuleError, match=message):
    ...
    def test_rule_metadata_validates_rule_code(self) -> None:
        ...
        with pytest.raises(TypeError, match="missing 1 required positional argument: 'stable_since'"):
            RuleMetadata(
                code=RuleCode("PDF101"), name="bad-rule", message="Bad rule", fix_availability=FixAvailability.ALWAYS, setting_effects=(), incompatible_with=(), check_kind=RuleCheckKind.STANDARD
        ...
        with pytest.raises(TypeError, match="missing 1 required positional argument: 'incompatible_with'"):
            RuleMetadata(
                code=RuleCode("PDF101"), name="bad-rule", message="Bad rule", fix_availability=FixAvailability.ALWAYS, stable_since="1.0.0", setting_effects=(), check_kind=RuleCheckKind.STANDARD
            )  # ty: ignore[missing-argument]
    ...
    def test_builtin_rule_setting_effect_matrix(self) -> None:
        for rule_class in rule_collection.RULE_COLLECTION.rules:
            for setting_effects in rule_class.meta.setting_effects:
                if setting_effects.setting == "docstring_convention":
                    for effect_values in setting_effects.effects:
                        assert effect_values.effect == RuleSettingEffect.IGNORED
    ...
    def test_builtin_docstring_convention_exact_opt_in_rules_are_exactly_selectable(self) -> None:
        ...
        exact_opt_in_codes: list[str] = [
            rule_class.meta.code.tag for rule_class in rule_collection.RULE_COLLECTION.rules if _ignored_docstring_conventions(rule_class.meta) == frozenset(DocstringConvention)
        ]

        assert exact_opt_in_codes
    ...
    def test_select_rules_requires_exact_selection_for_require_explicit_rules(self) -> None:
        ...
        for code in require_explicit_codes:
            ...
            assert code in tuple(rule.rule.code.tag for rule in rules_selection.select_rules(CheckSettings(select=(code,))).rules)
            assert code in tuple(rule.rule.code.tag for rule in rules_selection.select_rules(CheckSettings(extend_select=(code,))).rules)
    ...
    def test_select_rules_combines_multiple_setting_effects_with_disabled_precedence(self) -> None:
        ...
        disabled = rules_selection.select_rules(
            CheckSettings(select=("TST002",), docstring_convention=DocstringConvention.GOOGLE, docstring_parse_tables=False), collection=setting_effect_collection()
        )
    ...
    def test_ruff_spec_per_file_ignores_are_auto_config_directory_relative(self) -> None:
        ...
        assert self._active_rule_tags_for_path(matching_profile, target) == ()
        assert self._active_rule_tags_for_path(non_matching_profile, target) == ("PDF101",)
        assert self._active_rule_tags_for_path(bare_star_profile, target) == ()
        assert self._active_rule_tags_for_path(bare_literal_profile, target) == ()
    ...
    def test_ruff_spec_explicit_config_per_file_ignores_are_current_directory_relative(self) -> None:
        ...
        assert self._active_rule_tags_for_path(matching_profile, target) == ()
        assert self._active_rule_tags_for_path(non_matching_profile, target) == ("PDF101",)
        assert self._active_rule_tags_for_path(changed_pattern_profile, target) == ()
    ...
    def test_ruff_spec_per_file_ignores_apply_to_explicit_files(self) -> None:
        ...
        assert self._active_rule_tags_for_path(selection.profile_for_path(str(target)), target) == ()
